 ...
 * <p>
 * If data should be left in the database, call the setComplete() method in each test.
 * The defaultRollback() property, which defaults to true, determines whether
 * transactions will complete by default.
 * <p>
 * Requires a single bean in the context implementing the PlatformTransactionManager interface.
 * This will be set by the superclass's Dependency Injection mechanism. If using
 * the superclass's Field Injection mechanism, the implementation should be named "transactionManager".
 * This mechanism allows the use of this superclass even when there's more than one
 * transction manager in the context.
 * @author Rod Johnson
 */
public abstract class AbstractTransactionalSpringContextTests extends AbstractDependencyInjectionSpringContextTests {
    ...
    protected AbstractTransactionalSpringContextTests(String s) {
        super(s);
    }

    protected AbstractTransactionalSpringContextTests() {
        super();
    }
	...
	/**
	 * @see org.springframework.test.AbstractSpringContextTests#onSetUp()
	 */
	protected final void onSetUp() throws Exception {
        complete = !defaultRollback;
        // Start a transaction
        this.txStatus = transactionManager.getTransaction(new DefaultTransactionDefinition());
        
       // this.jdbcTemplate = (JdbcTemplate) applicationContext.getBean("jdbcTemplate");
        
        log.info("Began transaction: transaction manager=[" + transactionManager + "]; defaultCommit=" + complete);
