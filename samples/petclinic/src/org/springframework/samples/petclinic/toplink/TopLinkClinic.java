 ...
 * <p>
 * The mappings are defined in "toplink-mappings.xml"; session data is specified
 * in "toplink-sessions.xml" (both are located in the root of the class path).
 *
...
public class TopLinkClinic extends TopLinkDaoSupport implements Clinic {
	...
	public TopLinkClinic() {
		...
		this.getPetTypesQuery.addOrdering(this.getPetTypesQuery.getExpressionBuilder().get("name").ascending());
		...
		this.findOwnersQuery.setSelectionCriteria(builder.get("lastName").like(builder.getParameter("LastName")));
	...
	public Collection<Vet> getVets() throws DataAccessException {
		return (Collection<Vet>) getTopLinkTemplate().executeQuery(this.getVetsQuery);
	}
	...
	public Collection<PetType> getPetTypes() throws DataAccessException {
		return (Collection<PetType>) getTopLinkTemplate().executeQuery(this.getPetTypesQuery);
	}

	/**
	 * Return a set of Owner objects from the shared cache. Uses a "LASTNAME
	 * LIKE arg%" query.
	 */
	@Transactional(readOnly = true)
	@SuppressWarnings("unchecked")
	public Collection<Owner> findOwners(final String lastName) throws DataAccessException {
		return (Collection<Owner>) getTopLinkTemplate().executeQuery(this.findOwnersQuery,
				new Object[] { lastName + "%" });
	}
	...
	public void storeOwner(Owner owner) throws DataAccessException {
		// Note: TopLink's merge operation does not reassociate the object with
		// the current TopLink Session. Instead, it will always copy the state
		// over to a registered representation of the entity. In case of a new
		// entity, it will register a copy as well, but will also update the id
