...
configurations for JDBC, Hibernate, Oracle TopLink and JPA, with HSQLDB and
MySQL as target databases. The default PetClinic configuration is JDBC on
HSQLDB. See "WEB-INF/web.xml" and "WEB-INF/applicationContext-*.xml" for
details; a simple comment change in web.xml switches between the data access
strategies.
...
Note that special setup is necessary on WebLogic <= 8.1 and on JBoss: see
"jmxExporter" definition in "applicationContext-jdbc.xml" for details!
...
For standard TopLink, however, only a minimal "toplink-api.jar" is included in
the Spring distribution.

Thus to run or test PetClinic with TopLink, download TopLink 10.1.3 or higher
from the Oracle website (http://www.oracle.com/technology/products/ias/toplink/),
install it, and copy "toplink.jar" (located in the "jlib" directory of the
TopLink archive) into Spring's "lib/toplink" directory. TopLink uses the
OracleAS XML Parser for Java v2, which is packaged as "xmlparserv2.jar" in the
"lib" directory of an OracleAS installation. If you do not have access to
"xmlparserv2.jar", you can alternatively use "doctools.jar" from the
"doc/javadoc" directory of your TopLink archive, as this JAR also contains the
OracleAS XML Parser classes. Once you have located an appropriate OracleAS XML
Parser JAR, copy it (i.e., xmlparserv2.jar or doctools.jar) to into Spring's
"lib/toplink" directory.
...
TopLink TopLinkTransactionManager; for JPA, JpaTransactionManager. Those local
strategies allow for working with any locally defined DataSource.

Note that in the default case, the sample configurations specify Spring's
non-pooling DriverManagerDataSource as local DataSource. You can change the
DataSource definition to a Commons DBCP BasicDataSource, for example, to get
...
You can also invoke an existing installation of Ant, with this directory as
execution directory. Note that you need to do this to execute the "tests"
...
For MySQL, you'll need to use the corresponding schema and load scripts in the
"db/mysql" subdirectory. In the local case, the JDBC settings can be adapted in
"WEB-INF/jdbc.properties". With JTA, you need to set up corresponding
DataSources in your J2EE container.
