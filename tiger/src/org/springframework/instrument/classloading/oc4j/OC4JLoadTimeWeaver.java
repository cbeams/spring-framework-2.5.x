...
/**
 * LoadTimeWeaver suitable for OC4J. Many thanks to Mike Keith for assistance.
 * 
 * @author Costin Leau
 * 
 */
public class OC4JLoadTimeWeaver extends AbstractLoadTimeWeaver {
	...
	public void addClassFileTransformer(ClassFileTransformer classFileTransformer) {
		...
		if (classLoader.getPreprocessor() == null) {
			...
			if (debug)
				log.debug("adding classFileTransformer (first one) " + processor);
			...
			if (debug)
				log.debug("chaining classFileTransformer " + processor);
		}
	}

	@Override
	public ClassLoader getThrowawayClassLoader() {
		return classLoader.copy(false, false);
	}
}
