...
/**
 * Struts Action that acts a front controller entry point into the web flow system. Typically, a single FlowAction
 * manages all flow executions by parameterization with the appropriate <code>flowId</code> in views that start new
 * flow executions. Alternatively, a FlowAction may exist per top-level (root) flow definition in the application.
 * <p>
 * Requests are managed by and delegated to a {@link HttpServletFlowExecutionManager}, allowing reuse of common front
 * flow controller logic in other environments. Consult the JavaDoc of that class for more information on how requests
 * are processed.
 * <p>
 * This class also is aware of the <code>BindingActionForm</code> adapter, which adapts Spring's data binding
 * infrastructure (based on POJO binding, a standard Errors interface, and property editor type conversion) to the
 * Struts action form model. This gives backend web-tier developers full support for POJO-based binding with minimal
 * hassel, while still providing consistency to view developers who already have a lot of experience with Struts for
 * markup and request dispatching.
 * <p>
 * Below is an example <code>struts-config.xml</code> configuration for a FlowAction that fronts a single top-level
 * flow:
 * 
 * <pre>
 * 
 ...
 * </pre>
 * 
 * This example associates the logical request URL <code>/userRegistration.do</code> with the <code>Flow</code>
 * indentified by the id <code>user.Registration</code>. Alternatively, the <code>flowId</code> could have been
 * left blank and provided in dynamic fashion by the views (allowing a single <code>FlowAction</code> to manage any
 * number of flow executions). A binding action form instance is set in request scope, acting as an adapter enabling
 * POJO-based binding and validation with Spring.
 * <p>
 ...
 * <li>Use of the <code>BindingActionForm</code> requires some minor setup in <code>struts-config.xml</code>.
 * Specifically:
 * <ol>
 * <li>A custom BindingActionForm-aware request processor is needed, to defer form population:
 * 
 * <pre>
 * 
 ...
 * </pre>
 * 
 * <li>A <code>BindingPlugin</code> is needed, to plugin an Errors-aware <code>jakarta-commons-beanutils</code>
 * adapter:
 * 
 * <pre>
 * 
 ...
 * </ol>
 * </ul>
 * The benefits here are substantial: developers now have a powerful web flow capability integrated with Struts, with a
...
public class FlowAction extends TemplateAction {
	...
	/**
	 * Returns the flow locator used to lookup flows by id. Defaults to {@link BeanFactoryFlowServiceLocator}.
	 */
	...
	/**
	 * Get the flow id from given action mapping, which should be of type <code>FlowActionMapping</code>.
	 */
	...
	/**
	 * Creates the default flow execution manager. Subclasses can override this to return a specialized manager.
	 */
	...
	/**
	 * Creates a flow execution listener that takes a Spring Errors instance supporting POJO-based data binding in
	 * request scope under a well-defined name and adapts it to the Struts Action form model.
	 * 
	 * @param request
	 *            the request
	 * @param form
	...
	/**
	 * Return a Struts ActionForward given a ViewDescriptor. We need to add all attributes from the ViewDescriptor as
	 * request attributes.
	 */
