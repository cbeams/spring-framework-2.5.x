...
/**
 * Simple attributes mapper that allows mappings to be configured in the Spring application
 * context.
 * 
 * <p>
 ...
 * <td>empty</td>
 * <td>Mappings executed when mapping parent flow data <i>to </i> a newly spawned sub flow.
 * The provided list contains the names of the attributes in the parent to pass to the subflow
 * for access. The same name is used in both parent flow and sub flow model.</td>
 * </tr>
 * <tr>
 * <td>toMappingsMap</td>
 * <td>empty</td>
 * <td>Mappings executed when mapping parent flow data <i>to </i> a newly spawned sub flow.
 * The keys in given map are the names of entries in the parent model that will be mapped. The
 * value associated with a key is the name of the target entry that will be placed in the
 * subflow model.</td>
 * </tr>
 * <tr>
 * <td>fromMappingsList</td>
 * <td>empty</td>
 * <td>Mappings executed when mapping subflow data <i>from </i> the subflow back to the parent
 * flow (once the subflow ends and the parent flow resumes). The provided list contains the
 * names of the attributes in the subflow to pass to the parent for access. The same name is
 * used in both parent flow and sub flow model.</td>
 * </tr>
 * <tr>
 * <td>fromMappingsMap</td>
 * <td>empty</td>
 * <td>Mappings executed when mapping subflow flow data <i>from </i> the subflow back to the
 * parent flow (once the subflow ends and the parent flow resumes). The keys in given map are
 * the names of entries in the parent model that will be mapped. The value associated with a
 * key is the name of the target entry that will be placed in the subflow model.</td>
 * </tr>
 * </table>
 * 
 * <p>
 * The mappings defined using the above configuration properties fully support bean property
 * access. So an entry name in a mapping can either be "beanName" or "beanName.propName".
 * Nested property values are also supported ("beanName.propName.propName").
 * </p>
 * 
...
public class ParameterizableFlowAttributesMapper implements FlowAttributesMapper, Serializable {
    ...
    /**
     * Set the mappings that will be executed when mapping model data <i>to </i> a sub flow.
     * Each list item must be a String, a List, or a Map. If the list item is a simple String
     * value, the attribute will be mapped as having the same name in the parent flow and in
     * the child flow. If the list item is a Map, each map entry must be a String key naming
     * the attribute in the parent flow, and a String value naming the attribute in the child
     * flow. If the list item is itself a List, then that list is itself evaluated recursively,
     * and must itself contain Strings, Lists, or Maps.
     * 
     * <p>Only <strong>one</strong> of setToMappings or setToMappingsMap must be called. 
     */
    ...
    /**
     * <p>Set the mappings that will be executed when mapping model data <i>to</i> the sub flow.
     * This is essentially a just a short form of calling 
     * @link ParameterizableFlowAttributesMapper#setToMappings(List) with a List containing one
     * item which is a Map. Each map entry must be a String key naming the attribute in the parent
     * flow, and a String value naming the attribute in the child flow.
     * </p>
     * 
     * <p>Only <strong>one</strong> of setToMappings or setToMappingsMap must be called. 
     */
    ...
    /**
     * Set the mappings that will be executed when mapping model data <i>from</i> the sub flow.
     * Each list item must be a String, a List, or a Map. If the list item is a simple String
     * value, the attribute will be mapped as having the same name in the parent flow and in
     * the child flow. If the list item is a Map, each map entry must be a String key naming
     * the attribute in the child flow, and a String value naming the attribute in the parent
     * flow. If the list item is itself a List, then that list is itself evaluated recursively,
     * and must itself contain Strings, Lists, or Maps.
     * 
     * <p>Only <strong>one</strong> of setFromMappings or setFromMappingsMap must be called. 
     */
    ...
    /**
     * <p>Set the mappings that will be executed when mapping model data <i>from</i> the sub flow.
     * This is essentially a just a short form of calling 
     * @link ParameterizableFlowAttributesMapper#setFromMappings(List) with a List containing one
     * item which is a Map. Each map entry must be a String key naming the attribute in the child
     * flow, and a String value naming the attribute in the parent flow.
     * </p>
     * 
     * <p>Only <strong>one</strong> of setToMappings or setToMappingsMap must be called. 
     */
    ...
    private void putListMappings(Map map, List mappingsList) {
        ...
        while (it.hasNext()) {
            ...
            if (key instanceof List) {
                ...
                // we could just add the map into the other, but want to validate key and value
                // types!
    ...
    public Map createSpawnedSubFlowAttributesMap(AttributesAccessor parentFlowAttributes) {
        ...
        map(parentFlowAttributes, new MapAttributesAccessorAdapter(subFlowAttributes),
                toMappings);
            ...
            while (fromNames.hasNext()) {
                ...
                if (idx != -1) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Mapping bean property attribute from path '" + fromName
                                + "' to path '" + toName + "' with value '" + fromValue + "'");
                    }
                    ...
                    if (fromValue == null && !from.containsAttribute(fromName)) {
                        if (isMapMissingAttributesToNull()) {
                            if (logger.isDebugEnabled()) {
                                logger
                                        .debug("No value exists for attribute '"
                                                + fromName
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("Mapping attribute from name '" + fromName
                                    + "' to name '" + toName + "' with value '" + fromValue
                                    + "'");
                        }
    ...
    /**
     * <p>
     * Create a new bean wrapper wrapping given object. Can be redefined in subclasses in case
     * special property editors need to be registered or when other similar tuning is required.
     */
