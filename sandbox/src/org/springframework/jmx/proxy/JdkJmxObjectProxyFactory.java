...
public class JdkJmxObjectProxyFactory extends AbstractJmxObjectProxyFactory
	...
	private class JmxInvocationHandler implements InvocationHandler {
		...
		public JmxInvocationHandler(MBeanServerConnection server,
				ObjectName objectName) {
			this.server = server;
		...
		private Object invokeMethod(Method method, Object[] args)
				throws Exception {
			InternalMethodCacheKey key = new InternalMethodCacheKey(method
					.getName(), method.getParameterTypes());
			MBeanOperationInfo info = (MBeanOperationInfo) allowedOperations
					.get(key);
			...
			if (info == null) {
				...
				return server.invoke(objectName, method.getName(), args,
						signature);
			}
