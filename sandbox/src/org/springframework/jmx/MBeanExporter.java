...
public class MBeanExporter implements InitializingBean, DisposableBean, BeanFactoryAware {
    ...
    protected void registerBeans() throws JMException {
        ...
        if (this.beanFactory != null) {
            logger.info("Starting autodetect phases.");

            // autodetect any beans that are already MBeans
            logger.info("Autodetecting user-defined MBeans");
            autodetectMBeans();
    ...
    private void autodetectMBeans() {
        autodetect(new AutodetectCallback() {
            public boolean include(String beanName, Class beanClass) {
                boolean retVal = JmxUtils.isMBean(beanClass);

                if (retVal && logger.isInfoEnabled()) {
                    logger.debug("MBean with bean Name: " + beanName +
    ...
    private void autodetectBeans() {
        final AutodetectCapableModelMBeanInfoAssembler autodetectAssembler =
                (AutodetectCapableModelMBeanInfoAssembler) this.assembler;

        autodetect(new AutodetectCallback() {
            public boolean include(String beanName, Class beanClass) {
                boolean retVal = beanClass != null && autodetectAssembler.includeBean(beanName, beanClass);
     ...
     * @param callback the <code>AutodetectCallback</code> to use when deciding whether to include
     * a bean or not.
     */
    ...
    /**
     * Callback interface used during autodetection process.
     */
    private static interface AutodetectCallback {
         ...
         * @param beanClass the <code>Class</code> of the bean.
         * @return <code>true</code> to include the bean, else <code>false</code>.
         */
