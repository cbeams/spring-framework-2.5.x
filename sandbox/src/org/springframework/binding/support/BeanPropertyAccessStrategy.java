...
public class BeanPropertyAccessStrategy implements MutablePropertyAccessStrategy {
	private static final Log logger = LogFactory.getLog(BeanPropertyAccessStrategy.class);
	...
	public BeanPropertyAccessStrategy(Object bean) {
		this((bean instanceof BoundValueModel ? (BoundValueModel) bean : createValueHolder(bean)));
	}

	private static BoundValueModel createValueHolder(Object bean) {
		Assert.notNull(bean, "The bean value must not be null--otherwise I won't know what class the bean is!");
	...
	public BeanPropertyAccessStrategy(final BoundValueModel beanHolder) {
		...
		Assert.notNull(beanHolder.getValue(),
				"The bean holder value must not be null--otherwise I won't know what class the bean is!");
		if (logger.isDebugEnabled()) {
			logger.debug("[Subscribing to mutable bean holder; bean value=" + beanHolder.getValue() + "]");
		}
	...
	private class DomainObjectChangeHandler implements ValueChangeListener {
		public void valueChanged() {
			if (logger.isDebugEnabled()) {
				logger.debug("[" + ObjectUtils.getIdentityHexString(this) + " - Backing domain object of class "
						+ (beanHolder.getValue() != null ? beanHolder.getValue().getClass().getName() : "null")
		...
		private void updateBeanWrapper(Object newValue) {
			if (beanHolder.getValue() != null) {
				if (logger.isDebugEnabled()) {
					logger.debug("[Setting the enclosed bean wrapper's target object; new value='" + newValue + "']");
				}
				beanWrapper.setWrappedInstance(newValue);
			}
			else {
				throw new IllegalArgumentException("Held bean value cannot be null!");
			}
		}
	}

	private BeanPropertyAccessStrategy(BoundValueModel nestedDomainObjectHolder, String nestedPropertyName,
			final BeanPropertyAccessStrategy parent) {
	...
	public BoundValueModel getPropertyValueModel(String propertyPath) throws BeansException {
		if (logger.isDebugEnabled()) {
			logger.debug("Retrieving property value model for path '" + propertyPath + "'");
		}
		BeanPropertyAccessStrategy nestedAccessor = (BeanPropertyAccessStrategy) getPropertyAccessStrategyForPath(propertyPath);
		return nestedAccessor.getOrCreateValueModel(getFinalPath(nestedAccessor, propertyPath));
	}
	...
	public MutablePropertyAccessStrategy getPropertyAccessStrategyForPath(String propertyPath) throws BeansException {
		...
		if (pos > -1) {
			...
			String remainingNestedPropertyPath = propertyPath.substring(pos + 1);
	...
	protected static int getNestedPropertySeparatorIndex(String propertyPath, boolean last) {
		boolean inKey = false;
	 ...
	 */
	protected PropertyAccessStrategy getNestedPropertyAccessStrategy(String nestedPropertyName) throws BeansException {
		...
		if (nestedAccessor == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("Creating new nested BeanPropertyAccessor for property '" + canonicalName + "'");
			}
			BoundValueModel propertyValueHolder = getOrCreateValueModel(nestedPropertyName);
			nestedAccessor = new BeanPropertyAccessStrategy(propertyValueHolder, this.nestedPath + canonicalName
					+ PropertyAccessor.NESTED_PROPERTY_SEPARATOR, this);
			this.nestedPropertyAccessors.put(canonicalName, nestedAccessor);
		}
		else {
			if (logger.isDebugEnabled()) {
				logger.debug("Using cached nested BeanPropertyAccessor for property '" + canonicalName + "'");
			}
		}
	...
	protected BoundValueModel getOrCreateValueModel(String propertyName) {
		...
		BoundValueModel propertyValueHolder = (BoundValueModel) propertyValueModels.get(propertyName);
	...
	protected String[] getPropertyNameTokens(String propertyPath) {
		...
		String key = null;
		int keyStart = propertyPath.indexOf(PropertyAccessor.PROPERTY_KEY_PREFIX);
		if (keyStart != -1 && propertyPath.endsWith(PropertyAccessor.PROPERTY_KEY_SUFFIX)) {
			actualName = propertyPath.substring(0, keyStart);
			key = propertyPath.substring(keyStart + 1, propertyPath.length() - 1);
		...
		if (key != null) {
			canonicalName += PropertyAccessor.PROPERTY_KEY_PREFIX + key + PropertyAccessor.PROPERTY_KEY_SUFFIX;
		}
	...
	public void addPropertyChangeListener(PropertyChangeListener l) {
		if (beanHolder.getValue() instanceof PropertyChangePublisher) {
			PropertyChangePublisher p = (PropertyChangePublisher) beanHolder.getValue();
	...
	public void registerCustomEditor(String propertyName, PropertyEditor propertyEditor) {
		this.beanWrapper.registerCustomEditor(getMetadataAccessStrategy().getPropertyType(propertyName), propertyName,
				propertyEditor);
	}

	public PropertyEditor findCustomEditor(String propertyName) {
		return this.beanWrapper.findCustomEditor(getMetadataAccessStrategy().getPropertyType(propertyName),
				propertyName);
	}

	public PropertyMetadataAccessStrategy getMetadataAccessStrategy() {
		if (metaAspectAccessor == null) {
			this.metaAspectAccessor = new BeanPropertyMetaAspectAccessor(beanWrapper);
		}
		return metaAspectAccessor;
	}

	private static class BeanPropertyMetaAspectAccessor implements PropertyMetadataAccessStrategy {
		...
		public boolean isDate(String propertyName) {
			return Date.class.isAssignableFrom(beanWrapper.getPropertyDescriptor(propertyName).getPropertyType());
		}

		public boolean isEnumeration(String propertyName) {
			return LabeledEnum.class
					.isAssignableFrom(beanWrapper.getPropertyDescriptor(propertyName).getPropertyType());
		}
	...
	public String toString() {
		return new ToStringCreator(this).append("beanHolder", beanHolder).toString();
	}
