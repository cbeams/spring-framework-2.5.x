...
public class SingletonServiceLocatorLoader implements BeanFactoryPostProcessor, Ordered {
	...
	private Method getStaticLoadMethod(Class globalLocatorClass, Object instance) {
		...
		Method method = ClassUtils.getStaticMethod(globalLocatorClass, loadMethodName, args);
		if (method == null) {
			Class[] interfaces = instance.getClass().getInterfaces();
			for (int i = 0; i < interfaces.length; i++) {
				args[0] = interfaces[i];
				method = ClassUtils.getStaticMethod(globalLocatorClass, loadMethodName, args);
				if (method != null) {
					break;
				}
			}
