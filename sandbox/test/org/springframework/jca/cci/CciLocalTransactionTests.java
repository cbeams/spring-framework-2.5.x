...
import org.springframework.jca.cci.connection.CciTransactionManager;
...
public class CciLocalTransactionTests extends TestCase {
	 ...
	 * LocalTransaction when CciTransactionManager is specified as
	 * transaction manager.
	...
	public void testLocalTransactionCommit() throws ResourceException {
		...
		CciTransactionManager tm = new CciTransactionManager();
		...
		Object result = tt.execute(new TransactionCallback() {
			public Object doInTransaction(TransactionStatus status) {
				...
				return ct.execute(new DefaultCciSpecsHolder(null,interactionSpec),record,record);
			}
		});
		...
		try {
			Object result = tt.execute(new TransactionCallback() {
				public Object doInTransaction(TransactionStatus status) {
					assertTrue("Has thread connection", TransactionSynchronizationManager.hasResource(connectionFactory));
					CciTemplate ct = new CciTemplate(connectionFactory);
					Object obj = ct.execute(new DefaultCciSpecsHolder(null,interactionSpec),record,record);
