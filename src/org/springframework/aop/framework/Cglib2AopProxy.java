...
public class Cglib2AopProxy implements AopProxy, Serializable {
    ...
    public Object getProxy(ClassLoader cl) {
        ...
        try {
            ...
            for (int x = 0; x < callbacks.length; x++) {
                if (callbacks[x] instanceof AbstractInterceptor) {
        ...
        if (isStatic && isFrozen) {
            ...
            for (int x = 0; x < methods.length; x++) {
                ...
                List chain = advised.getAdvisorChainFactory().getInterceptorsAndDynamicInterceptionAdvice(
                        advised, null, methods[x], rootClass);
    ...
    private class ProxyCallbackFilter implements CallbackFilter {
        ...
        public int accept(Method method) {
            ...
            List chain = advised.getAdvisorChainFactory().getInterceptorsAndDynamicInterceptionAdvice(
                    advised, null, method, targetClass);
            ...
            if (haveAdvice) {
                ...
                if (isStatic && isFrozen
                    ...
                    if (logger.isInfoEnabled()) {
                        logger.info("Unable to apply any optimisations to advised method "
                                + method + " - using AOP_PROXY");
                    }
                    return AOP_PROXY;
                }
        ...
        public boolean equals(Object other) {
            ...
            if (advised.getTargetSource().isStatic() != otherCallbackFilter.advised.getTargetSource().isStatic()) {
                return false;
            }

            return (AopProxyUtils.equalsProxiedInterfaces(advised,
                    otherCallbackFilter.advised) & AopProxyUtils.equalsAdvisors(
                    advised, otherCallbackFilter.advised));
        }
