 ...
 * obtained from a Spring ApplicationContext. The handler in the context may
 * nest other handlers (via constructor injection) and so on, in standard JSF
 * fashion, forming a chain. If the last nested handler is an instance of
 * {@link org.springframework.web.jsf.navigation.JsfChainingNavigationHandler},
 * control will return back to the original handler that was passed in by JSF to
 * the constructor of this class. Normally the latter handler will be the
 * default JSF NavigationHandler, or any previous NavigationHandler defined in
 * faces-config.xml.
 * <p>
 * <p>
 * Configure this handler in your <code>faces-config.xml</code> file as
 * follows:
 * 
 * <pre>
 *  &lt;navigation-handler&gt;
 ...
 * 
 * <pre>
 *  &lt;bean name=&quot;jsfNavigationHandler&quot; class=&quot;com.mycompany.MyNavigtionHandler&quot;/&gt;
 * </pre>
 * 
 * On the other hand, most NavigationHandlers that are not supposed to be the
 * sole NavigationHandler in the application are by convention expected to
 * provide a constructor which takes a subsequent NavigationHandler to delegate
 * to if the first class does not know how to handle navigation. In this
 * fashion, you may chain NavigationHandlers by using nested bean definitions.
 * As long as the last nested bean is an instance of
 * JsfChainingNavigationHandler, then delegation will chain back to the original
 * NavigationHandler instance that was passed to the constructor of
 * DelegatingNavigationHandler by JSF.
 * 
 * <pre>
...
public class DelegatingNavigationHandler extends NavigationHandler {
	 ...
	 * 
	 * @param navigationHandler Next NavigationHandler in the chain
	 */
	 ...
	 */
	protected NavigationHandler getNavigationHandler(final FacesContext facesContext) throws BeansException {
		if (first != null) {
			return first;
		}
	...
	static abstract class NavigationHandlerContext {
		...
		/**
		 * Return the NavigationHandler associated with the current thread, if
		 * any.
		 ...
		 * @param navigationHandler the current NavigationHandler, or
		 * <code>null</code> to reset the thread-bound NavigationHandler
		 */
